    ...
    if (in_array($extension_du_fichier, $extensions_autorisees)) {
        ...
        if (count($errors) < 1) {
            ...
            if (move_uploaded_file($dossier_temporaire, $dossier_uploads)) {
                ...
                $requete = $pdo->prepare('INSERT INTO slider (id_slider, nom, titre, description, image) VALUES(null,?,?,?,?)');
                $requete->execute([$_POST['nom'], $_POST['titre'],$_POST['description'] , $_FILES['fichier']['name']]);
                header('location: index.php');
            }
        } else {
...
<div id="main" class="text-center">
    ...
    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data">
        <fieldset>
            <legend>Ajouter un Slider</legend>
